...
2) `previous_usage[n,t]` - Integer variable denoting number of previous operation periods until
    time t in which the electrolyzer n has been switched on.
    TODO: `previous_usage[n,t]` can potentially be left as a continuous variable. Test if the
    computational performance with SCIP/Gurobi is better or worse.
3) `efficiency_penalty[n,t]` - Coefficient that accounts for drop in efficiency at time t due
...
function EMB.variables_node(m, 𝒩, 𝒯, node::Electrolyzer, modeltype::EnergyModel)
    ...
    @variable(m, usage_strat_per[𝒩ᴴ, 𝒯ᴵⁿᵛ] >= 0, Int)
...
function EMB.create_node(m, n::Electrolyzer, 𝒯, 𝒫)
    ...
    for t_inv ∈ 𝒯ᴵⁿᵛ
        @constraint(m,
            m[:usage_strat_per][n, t_inv] == 
                sum(m[:elect_on][n, t]*t.duration for t ∈ t_inv) *
                t_inv.duration
        )
        for t ∈ t_inv
            if (TS.isfirst(t))
                @constraint(m,
                    m[:previous_usage][n, t] ==
                        sum(m[:usage_strat_per][n, t_inv_prev] for t_inv_prev ∈ 𝒯ᴵⁿᵛ if is_prior(t_inv_prev, t_inv))
                )
            else
                @constraint(m,
                    m[:previous_usage][n, t] ==
                        m[:previous_usage][n, previous(t, 𝒯)] + 
                        previous(t, 𝒯).duration * m[:elect_on][n, previous(t, 𝒯)]
                )
            end
        end
    end
    ...
    @constraint(m, [t ∈ 𝒯], 
        m[:cap_use][n, t] <= 100*n.Cap[t]*m[:elect_on][n,t]
    )
    @constraint(m, [t ∈ 𝒯], 
        m[:cap_use][n, t] >= 0.001*m[:elect_on][n,t]
    )
